...
@section('content')
    <div class="mb-6">
        <h2 class="text-2xl font-semibold">Edit Layanan: {{ $service->name }}</h2>
    </div>
    ...
    <form action="{{ route('admin.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white dark:bg-gray-800 p-6 rounded shadow">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4"
                      class="w-full px-4 py-2 border rounded dark:bg-gray-700 dark:text-white dark:border-gray-600 @error('description') border-red-500 @enderror">{{ old('description', $service->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="unit" id="unit" value="{{ old('unit', $service->unit) }}"
        ...
        <div>
            <label class="inline-flex items-center space-x-2">
                <input type="checkbox" name="is_outside_area" value="1"
                       ...
                       @checked(old('is_outside_area', $service->is_outside_area))>
            ...
            @if ($service->image)
                <div class="mt-3">
                    <img id="image-preview"
                        ...
                        alt="Preview Gambar">
                    <p class="text-xs text-gray-500 mt-1">Gambar saat ini / preview</p>
                </div>
            @else
                <div class="mt-3">
